...
public:
  Container(int rsize, int csize, int rows, int cols, string message)
      : Window(rsize, csize, rows, cols, message) {
    box(window, 0, 0);
    wmove(window, 1, 2);
  ...
  void addContainer(string items) { containerContent.push_back(items); }
...
vector<string> Container::itemdb = {"Sword", "Armor", "Apple", "Potion",
                                    "Shield"};
