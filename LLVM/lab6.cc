...
bool swapping(string input, Container &container1, Container &container2) {
  ...
  for (auto iter = container1.getVectorContainer().begin();
       iter != container1.getVectorContainer().end(); iter++) {
    if (*iter == input) {
      swapped = true;
  ...
  if (!swapped)
    for (auto iter = container2.getVectorContainer().begin();
         iter != container2.getVectorContainer().end(); iter++) {
      if (*iter == input) {
        swapped = true;
